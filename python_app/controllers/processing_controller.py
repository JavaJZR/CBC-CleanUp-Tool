...
class ProcessingController:
        ...
        try:
            ...
            for idx, row in current_df.iterrows():
                ...
                if employee_number is not None and full_name is None:
                    full_name = self.get_full_name_from_pernr(
                ...
                self.update_employee_record(
                    current_df, idx, employee_number, full_name, resignation_date,
                    org_data, match_type, match_score
                )
                
                # Update progress
                progress = 20 + (idx / len(current_df)) * 70
                self.main_controller.update_progress(
                    progress, f"Processing row {idx + 1} of {len(current_df)}..."
                )
    ...
    def initialize_new_columns(self, df: pd.DataFrame):
        ...
        new_columns = [
            'PERNR', 'Full Name (From Masterlist)', 'Resignation Date',
    ...
    def get_full_name_from_pernr(self, employee_number: str, masterlist_current_df: Optional[pd.DataFrame], 
                                 masterlist_resigned_df: Optional[pd.DataFrame]) -> Optional[str]:
        ...
        if pernr_col is not None:
            ...
            if name_columns:
                ...
                if not match.empty:
                    ...
                    return masterlist_current_df.loc[original_idx, name_columns[0]]
            ...
            if date_columns:
                ...
                if pd.notna(raw_date):
                    try:
                        ...
                        return str(raw_date) if raw_date else None
                else:
                    return None
    ...
    def update_employee_record(self, df: pd.DataFrame, idx, employee_number: Optional[str], 
                              full_name: Optional[str], resignation_date: Optional[str], 
